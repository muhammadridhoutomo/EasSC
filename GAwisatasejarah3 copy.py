...
# =====================================================================
# BLOK 2: OPERATOR GA - VERSI 3 (ELITIS & PBX)
# =====================================================================

# 1. SELEKSI: Rank Selection
def seleksi(populasi, fitness_list):
    sorted_indices = np.argsort(fitness_list)
    ranks = np.arange(1, len(populasi) + 1)
    total_rank = sum(ranks)
    probabilitas = ranks / total_rank
    idx_terpilih = np.random.choice(sorted_indices, size=2, p=probabilitas, replace=False)
    return populasi[idx_terpilih[0]], populasi[idx_terpilih[1]]

# 2. CROSSOVER: Position Based Crossover (PBX)
def crossover(parent1, parent2, crossover_rate):
    ...
    # Ambil beberapa posisi acak
    num_positions = random.randint(1, max(1, size // 2))
    positions = random.sample(range(size), num_positions)
    
    # Masukkan gen dari parent 1
    for pos in positions:
        child[pos] = parent1[pos]
        
    # Isi sisa slot kosong dengan urutan dari parent 2
...
# 3. MUTASI: Scramble Mutation (Diperhalus)
def mutasi(individu, mutation_rate):
    if random.random() < mutation_rate:
        size = len(individu)
        if size > 4:
            # UBAH DARI (2, 4) MENJADI (4, 8)
            start = random.randint(0, size - 8)
            end = start + random.randint(4, 8)
            sub_route = individu[start:end]
            random.shuffle(sub_route)
            individu[start:end] = sub_route
        else:
...
for gen in range(generasi_maks):
    ...
    if current_best_distance < best_distance_overall:
        ...
        # WAJIB TAMBAH .copy() AGAR REKOR TERBAIK TIDAK RUSAK DI TENGAH JALAN
...
# 7. Simpan File Peta
nama_file_peta = 'Peta_Rute_Berurutan3.html'
